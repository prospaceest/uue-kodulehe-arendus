...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    title: tx(locale, 'Салон в Таллинне — Vana-Kalamaja 8–110', 'Salong Tallinnas — Vana-Kalamaja 8–110'),
    description: tx(locale, 'Салон PROSPACE в Таллинне — посмотрите образцы профилей, получите консультацию, заберите сами. Пн–Пт 10–17.', 'PROSPACE salong Tallinnas — vaata profiilide näidiseid, otsi nõu ja tule ise järele. Avatud E–R 10–17.'),
    ...
    <div>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: 'var(--border)', minHeight: 560 }}>
        <div style={{ padding: '72px 56px', borderRight: 'var(--border)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <div className="vp-eyebrow">{tx(locale, 'Салон · Таллинн · Vana-Kalamaja 8', 'Salong · Tallinn · Vana-Kalamaja 8')}</div>
          <div>
            <h1 className="vp-display" style={{ fontSize: 'clamp(64px, 9vw, 144px)', margin: '24px 0 0', lineHeight: 0.9 }}>
              {tx(locale, 'Приходите смотреть.', 'Tule näidiseid vaatama.')}
            </h1>
            <p style={{ fontSize: 18, lineHeight: 1.6, color: 'var(--ink-2)', maxWidth: 480, marginTop: 28 }}>
              {tx(locale, 'В нашем салоне можно вживую посмотреть все серии профилей, потрогать покрытия, сравнить цвета RAL. Рекомендуем заранее забронировать визит.', 'Meie salongis saad kõik profiiliseeriad oma silmaga üle vaadata, katteid katsuda, RAL-värve võrrelda. Soovitame külastuse ette broneerida.')}
            </p>
          </div>
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginTop: 40 }}>
            <a href={site.booking} target="_blank" rel="noopener" className="vp-btn vp-btn--lg">{tx(locale, 'Забронировать визит →', 'Broneeri visiit →')}</a>
            <Link href={lp(`/kontakt`, locale)} className="vp-btn vp-btn--ghost vp-btn--lg">{tx(locale, 'Написать нам', 'Kirjuta meile')}</Link>
          </div>
        </div>
      ...
      <section style={{ borderBottom: 'var(--border)', position: 'relative', height: 420 }}>
        <iframe
          title={tx(locale, 'Карта салона', 'Salongi asukoht')}
          src={site.osmEmbed}
          width="100%"
          height="420"
        ...
        >
          {tx(locale, 'Открыть в картах →', 'Ava kaardirakenduses →')}
        </a>
      </section>

      {/* What to see in salon */}
      <section style={{ padding: '72px 56px', borderBottom: 'var(--border)' }}>
        <div className="vp-eyebrow" style={{ marginBottom: 14 }}>{tx(locale, 'Что можно посмотреть', 'Mida saab näha')}</div>
        <h2 className="vp-display" style={{ fontSize: 64, margin: '0 0 48px', lineHeight: 0.95 }}>
          {tx(locale, 'Все серии в наличии.', 'Kõik seeriad kohapeal.')}
        </h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
          {[
            ...
            <div key={i} style={{ padding: '28px 24px', border: 'var(--border)' }}>
              ...
              <div className="vp-display" style={{ fontSize: 26, lineHeight: 1.05, marginBottom: 10 }}>{tx(locale, c.tRu, c.tEt)}</div>
              <div style={{ fontSize: 13, lineHeight: 1.55, color: 'var(--ink-2)' }}>{tx(locale, c.dRu, c.dEt)}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: '72px 56px', background: 'var(--paper-2)', borderBottom: 'var(--border)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'center' }}>
        <div>
          <div className="vp-eyebrow" style={{ marginBottom: 14 }}>{tx(locale, 'Забронировать визит', 'Broneeri visiit')}</div>
          <h2 className="vp-display" style={{ fontSize: 56, margin: 0, lineHeight: 0.95 }}>
            {tx(locale, '1 час · бесплатно.', '1 tund · tasuta.')}
          </h2>
          <p style={{ fontSize: 16, lineHeight: 1.6, color: 'var(--ink-2)', marginTop: 18 }}>
            {tx(locale, 'Персональная встреча со специалистом — покажем профили, ответим на вопросы по монтажу и подберём оттенок RAL.', 'Isiklik kohtumine spetsialistiga — näitame profiile, vastame paigalduse küsimustele ja aitame RAL tooni valida.')}
          </p>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <a href={site.booking} target="_blank" rel="noopener" className="vp-btn vp-btn--lg" style={{ textAlign: 'center' }}>
            {tx(locale, 'Забронировать онлайн →', 'Broneeri online →')}
          </a>
