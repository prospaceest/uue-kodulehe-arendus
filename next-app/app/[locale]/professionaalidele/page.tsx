  ...
  return (
    <div>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', borderBottom: 'var(--border)', minHeight: 600 }}>
        <div style={{ padding: '72px 56px', borderRight: 'var(--border)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <div className="vp-eyebrow">
            {tx('Партнёрская программа · Дизайнерам · Монтажникам · Реселлерам', 'Partneriprogramm · Sisearhitektidele · Paigaldajatele · Edasimüüjatele')}
          </div>
          <div>
            <h1 className="vp-display" style={{ fontSize: 'clamp(72px, 11vw, 168px)', margin: '24px 0 0', lineHeight: 0.9 }}>
              {tx('Сотрудничайте', 'Tee koostööd')}<br />
              <span style={{ fontFamily: "'Inter', serif", fontStyle: 'italic', fontWeight: 300, fontSize: '0.55em', letterSpacing: '-0.02em' }}>
                {tx('по лучшей цене.', 'parima hinnaga.')}
              </span>
            </h1>
            <p style={{ fontSize: 18, lineHeight: 1.6, color: 'var(--ink-2)', maxWidth: 520, marginTop: 28 }}>
              {tx('Присоединяйтесь к партнёрской сети PROSPACE — получите ', 'Liitu PROSPACE partnerite võrgustikuga — saad ')}
              <strong>{tx('персональную скидку', 'personaalse soodushinna')}</strong>
              {tx(', быстрые сроки доставки и техподдержку на каждом проекте.', ', kiired tarneajad ning tehnilise toe igal projektil.')}
            </p>
          </div>
          <div style={{ display: 'flex', gap: 14, marginTop: 40 }}>
            <a href="#b2b-form" className="vp-btn vp-btn--lg">{tx('Стать партнёром →', 'Liitu partneriks →')}</a>
            <Link href={lp(`/konto/login`, locale)} className="vp-btn vp-btn--ghost vp-btn--lg">{tx('Войти', 'Logi sisse')}</Link>
          </div>
        </div>
        <div className="vp-photo" style={{ backgroundImage: 'url("/assets/b2b-vannituba.jpg")', backgroundSize: 'cover', backgroundPosition: 'center' }}>
          <span className="label">{tx('b2b · монтажник в салоне', 'b2b · paigaldaja salongis')}</span>
        </div>
      </section>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', borderBottom: 'var(--border)' }}>
        {[
          ...
          <div key={i} style={{ padding: '48px 32px', borderRight: i < 3 ? 'var(--border)' : 'none', textAlign: 'center' }}>
            ...
            <div className="vp-eyebrow">{tx(b.tRu, b.tEt)}</div>
          </div>
        ))}
      </section>
      ...
      <section style={{ padding: '72px 56px', borderBottom: 'var(--border)' }}>
        <div className="vp-eyebrow" style={{ marginBottom: 14 }}>{tx('Что получает партнёр', 'Mida partner saab')}</div>
        <h2 className="vp-display" style={{ fontSize: 72, margin: '0 0 48px', lineHeight: 0.95, maxWidth: 920 }}>
          {tx('Конкретные преимущества,', 'Konkreetsed eelised,')}<br />
          <span style={{ fontFamily: "'Inter', serif", fontStyle: 'italic', fontWeight: 300, fontSize: '0.7em' }}>
            {tx('не просто обещания.', 'mitte ainult lubadused.')}
          </span>
        </h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 0, border: 'var(--border)' }}>
          {[
            ...
            <div key={b.n} style={{ padding: '36px', borderRight: i % 2 === 0 ? 'var(--border)' : 'none', borderBottom: i < arr.length - 1 ? 'var(--border)' : 'none', display: 'flex', gap: 24 }}>
              ...
              <div>
                <div className="vp-display" style={{ fontSize: 32, marginBottom: 8 }}>{tx(b.tRu, b.tEt)}</div>
                <div style={{ fontSize: 14, lineHeight: 1.6, color: 'var(--ink-2)' }}>{tx(b.dRu, b.dEt)}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <section style={{ padding: '72px 56px', background: 'var(--paper-2)', borderBottom: 'var(--border)' }}>
        <div className="vp-eyebrow" style={{ marginBottom: 14 }}>{tx('Как это работает', 'Kuidas käib')}</div>
        <h2 className="vp-display" style={{ fontSize: 64, margin: '0 0 40px', lineHeight: 0.95 }}>
          {tx('Регистрация за 5 минут.', 'Liitu 5 minutiga.')}
        </h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
          {[
            ...
            <div key={s.n} style={{ padding: '24px', border: 'var(--border)', background: 'var(--paper)' }}>
              ...
              <div className="vp-display" style={{ fontSize: 30, marginBottom: 8 }}>{tx(s.tRu, s.tEt)}</div>
              <div style={{ fontSize: 13, lineHeight: 1.55, color: 'var(--ink-2)' }}>{tx(s.dRu, s.dEt)}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="b2b-form" style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', borderBottom: 'var(--border)' }}>
        <div style={{ padding: '72px 56px', borderRight: 'var(--border)' }}>
          <div className="vp-eyebrow" style={{ marginBottom: 10 }}>{tx('Регистрация', 'Liitumine')}</div>
          <h2 className="vp-display" style={{ fontSize: 64, margin: '0 0 22px', lineHeight: 0.95 }}>
            {tx('Отправьте', 'Saada')}<br />{tx('анкету.', 'ankeet.')}
          </h2>
          <p style={{ fontSize: 16, lineHeight: 1.6, color: 'var(--ink-2)', maxWidth: 380 }}>
            {tx('Ответим в течение 24 ч (Пн–Пт). Регистрация бесплатна, скидки активируются сразу после подтверждения.', 'Vastame 24 h jooksul (E–R). Liitumine on tasuta, soodushindade aktiveerumine kohe peale kinnitust.')}
          </p>
          ...
          {submitted ? (
            <div style={{ padding: '56px 32px', border: 'var(--border)', background: 'var(--paper)', textAlign: 'center' }}>
              ...
              <h3 className="vp-display" style={{ fontSize: 42, margin: '0 0 16px', lineHeight: 0.95 }}>
                {tx('Спасибо!', 'Aitäh!')}<br />{tx('Анкета отправлена.', 'Ankeet saadetud.')}
              </h3>
              <p style={{ fontSize: 15, lineHeight: 1.65, color: 'var(--ink-2)', maxWidth: 420, margin: '0 auto 26px' }}>
                {tx('Ответим в течение 24 ч (Пн–Пт).', 'Vastame 24 h jooksul (E–R).')}
              </p>
              <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
                <Link href={lp(`/tooted`, locale)} className="vp-btn">{tx('Смотреть товары →', 'Sirvi tooteid →')}</Link>
                <button className="vp-btn vp-btn--ghost" onClick={() => setSubmitted(false)}>{tx('Отправить ещё анкету', 'Saada veel ankeet')}</button>
              </div>
            </div>
          ) : (
            <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <input className="vp-input" placeholder={`${tx('Компания', 'Ettevõte')} *`} required value={form.company} onChange={up('company')} />
              <input className="vp-input" placeholder={`${tx('Рег.№', 'Reg.nr')} *`} value={form.regNr} onChange={up('regNr')} />
              <input className="vp-input" placeholder={`${tx('Контактное лицо', 'Kontaktisik')} *`} required value={form.contact} onChange={up('contact')} />
              <input className="vp-input" placeholder={`${tx('Телефон', 'Telefon')} *`} required value={form.phone} onChange={up('phone')} />
              ...
              <select className="vp-input" style={{ gridColumn: '1 / -1', cursor: 'pointer' }} value={form.role} onChange={up('role')}>
                <option value="">{tx('Сфера деятельности — выберите...', 'Tegevusvaldkond — vali...')}</option>
                <option>{tx('Дизайнер интерьера', 'Sisearhitekt / disainer')}</option>
                <option>{tx('Монтажник / строитель', 'Paigaldaja / ehitaja')}</option>
                <option>{tx('Реселлер', 'Edasimüüja')}</option>
                <option>{tx('Архитектурное бюро', 'Arhitektuuribüroo')}</option>
                <option>{tx('Другое', 'Muu')}</option>
              </select>
              <input className="vp-input" placeholder={tx('Адрес сайта (напр. firma.ee)', 'Kodulehe aadress (nt firma.ee)')} style={{ gridColumn: '1 / -1' }} value={form.website} onChange={up('website')} />
              <textarea className="vp-input" rows={4} placeholder={tx('Краткое описание, текущие проекты...', 'Lühitutvustus, käimasolevad projektid...')} style={{ gridColumn: '1 / -1' }} value={form.description} onChange={up('description')} />
              <label style={{ gridColumn: '1 / -1', display: 'flex', gap: 10, fontSize: 13, alignItems: 'flex-start', cursor: 'pointer' }}>
                ...
                <span>{tx('Согласен с обработкой персональных данных по политике конфиденциальности', 'Nõustun isikuandmete töötlemisega vastavalt privaatsuspoliitikale')}</span>
              </label>
              <button type="submit" className="vp-btn vp-btn--lg vp-btn--block" style={{ gridColumn: '1 / -1', marginTop: 8 }} disabled={loading}>
                {loading ? (tx('Отправка…', 'Saadan…')) : (tx('Отправить анкету →', 'Saada ankeet →'))}
              </button>
            </form>
          )}
        </div>
      ...
      {/* WHY US */}
      <section style={{ padding: '72px 56px', borderBottom: 'var(--border)' }}>
        <div className="vp-eyebrow" style={{ marginBottom: 14 }}>{tx('Почему сотрудничать с нами', 'Miks meiega koostööd teha')}</div>
        <h2 className="vp-display" style={{ fontSize: 48, margin: '0 0 32px', lineHeight: 0.95, maxWidth: 920 }}>
          {tx('Товары, которые работают —', 'Tooted, mis töötavad —')}<br />
          {tx('клиенты, которые довольны.', 'kliendid, kes jäävad rahule.')}
        </h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
          {[
            ...
            <div key={i} style={{ padding: '32px 28px', border: 'var(--border)' }}>
              ...
              <div className="vp-display" style={{ fontSize: 28, lineHeight: 1.05, marginBottom: 12 }}>{tx(c.tRu, c.tEt)}</div>
              <div style={{ fontSize: 15, lineHeight: 1.55, color: 'var(--ink-2)' }}>{tx(c.dRu, c.dEt)}</div>
            </div>
          ))}
        </div>
      </section>
