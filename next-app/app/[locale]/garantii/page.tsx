...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    title: tx(locale, 'Гарантия и претензии', 'Garantii ja pretensioonid'),
    description: tx(locale, 'Условия гарантии на теневые профили — 5 лет против производственных дефектов. Как подать претензию в PROSPACE OÜ.', 'Varjuprofiilide garantiitingimused — 5 aastat tootmisdefektide vastu. Kuidas esitada pretensiooni PROSPACE OÜ-le.'),
    ...
    <div>
      <section style={{ padding: '72px 56px 32px', borderBottom: 'var(--border)' }}>
        <div className="vp-eyebrow" style={{ marginBottom: 10 }}>
          {tx(locale, 'Сервис · Гарантия', 'Klienditeenindus · Garantii')}
        </div>
        <h1 className="vp-display" style={{ fontSize: 'clamp(72px, 11vw, 168px)', margin: 0, lineHeight: 0.9 }}>
          {tx(locale, 'Гарантия', 'Garantii')}<br />
          <span style={{ fontFamily: "'Inter', serif", fontStyle: 'italic', fontWeight: 300, fontSize: '0.5em', letterSpacing: '-0.02em' }}>
            {tx(locale, 'на профиль и его покрытие.', 'profiilile ja selle värvile.')}
          </span>
        </h1>
        <p style={{ fontSize: 17, lineHeight: 1.6, color: 'var(--ink-2)', maxWidth: 720, marginTop: 22 }}>
          {tx(locale, 'Продаём только алюминиевые профили и покрываем гарантией только то, за что несём ответственность — сам профиль и его покрытие. LED-ленты, блоки питания и другую электронику не продаём и не гарантируем.', 'Müüme ainult alumiiniumprofiile ja katame garantiiga vaid seda, mille eest meie vastutame — profiili ennast ja selle värvi. LED-ribasid, toiteplokke ega muid elektroonika komponente me ei müü ega garanteeri.')}
        </p>
      </section>

      <section style={{ padding: '56px', borderBottom: 'var(--border)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 56 }}>
        <div>
          <div className="vp-eyebrow" style={{ marginBottom: 14 }}>{tx(locale, 'Что покрывает', 'Mis kehtib')}</div>
          <h2 className="vp-display" style={{ fontSize: 48, margin: '0 0 22px' }}>{tx(locale, 'Гарантия\nпокрывает.', 'Garantii\nkatab.')}</h2>
          <ul style={{ margin: 0, paddingLeft: 18, fontSize: 15, lineHeight: 1.8, color: 'var(--ink-2)' }}>
            <li>{tx(locale, 'Заводские дефекты', 'Tootmisdefektid')}</li>
            <li>{tx(locale, 'Коррозия и окисление в нормальных условиях', 'Korrosioon ja oksüdeerumine normaaltingimustes')}</li>
            <li>{tx(locale, 'Выцветание краски в помещениях', 'Värvi tuhmumine sisetingimustes')}</li>
          </ul>
        </div>
        <div>
          <div className="vp-eyebrow" style={{ marginBottom: 14 }}>{tx(locale, 'Что не покрывает', 'Mis ei kehti')}</div>
          <h2 className="vp-display" style={{ fontSize: 48, margin: '0 0 22px' }}>{tx(locale, 'Гарантия\nне покрывает.', 'Garantii\nei kata.')}</h2>
          <ul style={{ margin: 0, paddingLeft: 18, fontSize: 15, lineHeight: 1.8, color: 'var(--ink-2)' }}>
            <li>{tx(locale, 'Неправильный монтаж', 'Vale paigaldus')}</li>
            <li>{tx(locale, 'Механические повреждения после монтажа', 'Mehaanilised vigastused peale paigaldust')}</li>
            <li>{tx(locale, 'Повреждения от химикатов и растворителей', 'Kemikaalide, lahustite kahjustused')}</li>
            <li>{tx(locale, 'Естественный износ', 'Loomulik kulumine')}</li>
          </ul>
        </div>
      </section>

      <section style={{ padding: '56px', background: 'var(--paper-2)', borderBottom: 'var(--border)' }}>
        <div className="vp-eyebrow" style={{ marginBottom: 14 }}>{tx(locale, 'Подача претензии', 'Pretensiooni esitamine')}</div>
