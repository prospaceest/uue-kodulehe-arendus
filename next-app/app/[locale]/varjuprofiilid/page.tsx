...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const ru = locale === 'ru';
  return {
    title: tx(locale, 'Теневые профили – декоративные алюминиевые профили для потолка, стены, пола', 'Varjuprofiilid – dekoratiivsed alumiiniumprofiilid laele, seinale, põrandale'),
    description: tx(locale, 'Декоративные теневые профили без LED — чистая архитектурная теневая линия из алюминия. Для потолка, стены и пола. Любой оттенок RAL. Склад в Таллинне.', 'Dekoratiivsed varjuprofiilid ilma LED-ita — puhas arhitektuurne varjujoon alumiiniumist. Laele, seinale ja põrandale. Iga RAL-toon. Ladu Tallinnas.'),
    ...
    <div>
      ...
      <section style={{ padding: '32px 56px 0' }}>
        <div className="vp-eyebrow">
          ...
          {' / '}
          <span style={{ color: 'var(--ink)' }}>{tx(locale, 'Теневые профили', 'Varjuprofiilid')}</span>
        </div>
      </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64 }}>
          <div>
            <div className="vp-eyebrow" style={{ marginBottom: 14 }}>
              {tx(locale, 'Sammas 2 · Декоративные, без LED', 'Sammas 2 · Dekoratiivsed, ilma LED-ita')}
            </div>
            <h1 className="vp-display" style={{ fontSize: 'clamp(48px, 7vw, 96px)', margin: '0 0 24px', lineHeight: 0.9 }}>
              {tx(locale, 'Теневые профили.', 'Varjuprofiilid.')}
            </h1>
            <p style={{ fontSize: 17, lineHeight: 1.75, color: 'var(--ink-2)', maxWidth: 520, marginBottom: 24 }}>
              {tx(locale, 'Теневые профили (по-английски shadow gap profiles) — алюминиевые профили, создающие между двумя поверхностями намеренную, равномерную теневую линию без подсветки. Чисто архитектурная деталь. Используются там, где нужна визуальная граница — не освещение.', 'Varjuprofiilid (inglise k. shadow gap profiles) on alumiiniumprofiilid, mis loovad kahe pinna vahele tahtliku, ühtlase varjujoone — ilma valgustuseta. Puhtalt arhitektuurne detail. Kasutatakse seal, kus on vaja visuaalset eraldust — mitte valgustust.')}
            </p>
            <div style={{ padding: '16px 20px', border: 'var(--border)', background: 'var(--paper-2)', marginBottom: 32, fontFamily: 'JetBrains Mono, monospace', fontSize: 13 }}>
              {tx(locale, '«Все LED профили — теневые профили. Но не все теневые профили — LED.»', '"Kõik LED profiilid on varjuprofiilid. Aga kõik varjuprofiilid ei ole LED."')}
            </div>
            <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
              <Link href={lp(`/tooted`, locale)} className="vp-btn vp-btn--lg">{tx(locale, 'Все товары →', 'Vaata kõiki →')}</Link>
              <Link href={lp(`/led-varjuprofiilid`, locale)} className="vp-btn vp-btn--ghost">{tx(locale, 'LED версии →', 'LED versioonid →')}</Link>
            </div>
          </div>
          <div>
            <div className="vp-eyebrow" style={{ marginBottom: 14 }}>{tx(locale, 'Декоративные vs LED', 'Dekoratiivne vs LED')}</div>
            <div style={{ border: 'var(--border)' }}>
              {[
                { prop: tx(locale, 'LED-подсветка', 'LED-valgustus'), deco: tx(locale, '✗ нет', '✗ ei'), led: tx(locale, '✓ integreeritud', '✓ integreeritud') },
                { prop: tx(locale, 'Функция', 'Funktsioon'), deco: tx(locale, 'Архитект. деталь', 'Arhitektuurne detail'), led: tx(locale, 'Деталь + свет', 'Detail + valgus') },
                { prop: tx(locale, 'Монтаж', 'Paigaldus'), deco: tx(locale, 'Проще', 'Lihtsam'), led: tx(locale, 'Нужна проводка', 'Vajab juhtmestikku') },
                { prop: tx(locale, 'Цена', 'Hind'), deco: tx(locale, 'Ниже', 'Madalam'), led: tx(locale, 'Выше', 'Kõrgem') },
              ].map((row, i) => (
      ...
      <section style={{ padding: '64px 56px', borderBottom: 'var(--border)' }}>
        <div className="vp-eyebrow" style={{ marginBottom: 14 }}>
          {tx(locale, 'Декоративные по месту установки', 'Dekoratiivsed asukoha järgi')}
        </div>
        <h2 className="vp-display" style={{ fontSize: 64, margin: '0 0 40px', lineHeight: 0.92 }}>
          {tx(locale, 'Потолок, стена, пол.', 'Lagi, sein, põrand.')}
        </h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1, background: 'var(--ink)', border: 'var(--border)' }}>
          {SUBCATEGORIES.map((cat) => (
            <Link key={cat.slug} href={lp(`/varjuprofiilid/${cat.slug}`, locale)} style={{ display: 'block', background: 'var(--paper)', padding: '36px 32px', textDecoration: 'none', color: 'inherit' }}>
      ...
      {dekorProducts.length > 0 && (
        <section style={{ padding: '64px 56px', borderBottom: 'var(--border)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 32 }}>
            <div>
              <div className="vp-eyebrow" style={{ marginBottom: 8 }}>{tx(locale, 'Популярные деко профили', 'Populaarsed dekoratiivsed profiilid')}</div>
