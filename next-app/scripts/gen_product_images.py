#!/usr/bin/env python3
"""Genereerib lib/productImages.ts kureeritud andmestikust ja valideerib,
et iga viidatud failinimi assets/products/ kaustas päriselt olemas on.

Kureerimise reeglid (kasutaja soov, 2026-09):
- Poe grid-pilt (PRODUCT_IMAGES) = nurga-/paigaldusvaade. Mitte kunagi mõõtjoonis ega paljas plaan.
- Laeprofiilidel eelistatud kirjeldava nimega nurgavaade ({sku}-lae-varjuprofiil.webp) nagu AST30.
- Mõõtjoonised (PRODUCT_DIMENSIONS) EI kuvata poe üldvaates; galeriis alati viimasena.
"""
import os, sys

ASSETS = os.path.join(os.path.dirname(__file__), '..', 'public', 'assets', 'products')

# grid-pilt: None => tekst-fallback (nt lisatarvikud, ainult mõõtjoonisega tooted)
GRID = {
    # ── Laeprofiilid — eelistatud nurgavaade ──
    ...
    'KA1': 'KA1_2.jpg',
    'KA2': 'KA2_2.jpg',
    # ── Värvivariantide SKU-d — kasutavad emaprofiili õiget värvipilti ──
    ...
    # ── Pildita tooted (tekst-fallback) ──
    'DKP1010': None, 'DKP1515': None, 'DKP2020': None,
    'ASL302': None, 'ASL312': None, 'ASL300': None, 'ASL25': None, 'ASL40': None,
    'ASK100': None, 'P1': None, 'F40': None, 'F60': None, 'F80': None, 'F100': None,
    'HAJUTI-M': None,
}

# Galerii fotod (mitte-mõõtjoonised): paigaldus/foto/lifestyle, ühendusdetailid lõpupoole.
PHOTOS = {
...
# Mõõtjoonised — poe gridis mitte kunagi; galerii lõppu.
DIMS = {
...
def validate():
    ...
    missing = []
    for name, d in (('GRID', GRID), ('PHOTOS', PHOTOS), ('DIMS', DIMS)):
        for sku, v in d.items():
            files = [v] if isinstance(v, str) else (v or [])
            for f in files:
                if f and f not in existing:
                    missing.append(f"{name}[{sku}] -> {f}")
    return missing
...
def emit_record(name, typ, data):
    lines = [f"export const {name}: {typ} = {{"]
    for sku in data:
        v = data[sku]
        key = sku if sku.replace('_', '').isalnum() else f"'{sku}'"
        if "'" in sku or '-' in sku:
            key = f"'{sku}'"
        if isinstance(v, list):
            arr = ', '.join(ts_val(x) for x in v)
            lines.append(f"  {key}: [{arr}],")
        else:
            lines.append(f"  {key}: {ts_val(v)},")
    lines.append('};')
    return '\n'.join(lines)
...
def main():
    missing = validate()
    if missing:
        print('PUUDUVAD FAILID:', file=sys.stderr)
        for m in missing:
            print('  ' + m, file=sys.stderr)
        sys.exit(1)

    header = '''// Toote piltide kaardistus SKU -> failinimi (assets/products/ suhtes).
...
'''
    ...
    body = [
        header,
        emit_record('PRODUCT_IMAGES', 'Record<string, string | null>', GRID),
        '',
        emit_record('PRODUCT_PHOTOS', 'Record<string, string[]>', PHOTOS),
        '',
        emit_record('PRODUCT_DIMENSIONS', 'Record<string, string[]>', DIMS),
        '',
...
''',
    ...
    out = os.path.join(os.path.dirname(__file__), '..', 'lib', 'productImages.ts')
    with open(out, 'w') as fh:
        fh.write('\n'.join(body).rstrip() + '\n')
    print('OK — productImages.ts genereeritud. Kõik failid valideeritud.')
